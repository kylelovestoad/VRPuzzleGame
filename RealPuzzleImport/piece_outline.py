...
def flood_fill_piece_interiors(initial_background, rows, cols):
    ...
    cv2.floodFill(initial_background, flood_fill_mask, (0, 0), cleaned_background_marker, flags=flood_fill_flags)
...
def clean_piece_mask(noisy_piece_mask):
    num_labels, labels, stats, _ = cv2.connectedComponentsWithStats(noisy_piece_mask, connectivity=8)
...
def debug_pieces(image, piece_mask, contours):
    ...
    red = (0, 0, 255)
    all_contours = -1
    cv2.drawContours(output, contours, all_contours, color=red, thickness=2)

    hulls = []

    for con in contours:
        hulls.append(cv2.convexHull(con))
